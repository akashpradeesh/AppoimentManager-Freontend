    ...
    return (
        <TableContainer component={Paper}>
            ...
            {singleUserData.map((data,index)=>
                ...
                <TableCell align='right' name='date'>{(schedule[index] === false)?(<a><input type='date'  name="date" min={todayDate} onChange={(e)=>{handleDateChange(e,data.event)}}></input><Button onClick={()=>{handleDateOnChange(index)}}>save</Button></a>):(<a>{data.date}</a>)}</TableCell>
                ...
                <Select labelId='demo-simple-select-label' label='Action' value={action[index]} onChange={(e)=>handleChange(e,index)}>
                    ...
                    <Button onClick={()=>{handleOnClick(index,data.date,data.event)}}>save</Button>
                    </FormControl>
